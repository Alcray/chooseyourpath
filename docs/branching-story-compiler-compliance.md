...
| Provider interface | Partial | Routes depend on `VideoProvider`, and prompt language is provider-neutral. Completed output is treated as untrusted: exact `video/mp4`, valid base64, and nonempty decoded bytes are required before either extension or canonical R2 storage, and storage metadata is forced to MP4. The only implementation and provider video type are still Veo-specific. |
...
| Pause/resume | Implemented; newly compiled v2 media recheck pending | The active persistent video element retains native controls and node identity; the ending narration overlay cannot intercept those controls. The deterministic browser test verifies both routes with simulated media. A separate opt-in Chromium test selected a supplied completed schema `1.0` record, decoded all four stored Veo files, exercised both paths through the shared ending, and proved native pause/resume position retention. That test accelerates boundaries with synthetic `ended` events, so it does not prove natural end timing, handoff latency, or gaplessness. A newly rendered schema/prompt v2 story has not yet repeated the provider-media check. |
...
- native pause/resume and transition verification using newly rendered schema/prompt v2 video files;
...
**Do not deploy this compiler/prompt/schema revision yet.** Static, route-boundary, mocked-browser, legacy-compatibility, and stored native-media tests pass. The Moral Story Engine release rule still requires a live Gemini route test after schema/prompt changes, a real schema/prompt v2 provider exercise followed by native playback verification, and a decision on the remaining build-tool advisories before deployment. Even after those pass, production-grade visual QA, deletion/retention, and durable workflow gaps above must be accepted explicitly or implemented before treating the complete long-term architecture as delivered.
