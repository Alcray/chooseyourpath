...
2. Preserve these explicit stages: policy, premises, story graph, independent review, and shot manifest.
3. Keep deterministic validation separate from model judgment. Reject malformed stage output before advancing.
4. Keep provider-specific code behind `VideoProvider`. Compile canon IDs into provider prompts only after parent approval.
5. Store the complete versioned `StoryPackage`, not only the rendered prompts.
6. Add or update golden cases whenever policy, prompt text, schemas, state rules, scoring, or provider compilation changes.
7. Run the resource scripts and application tests before handoff.
...
Do not deploy a provider/prompt/schema change based only on static tests. Exercise the parent approval screen, progress states, both playback branches, pause/resume, and the shared ending.
